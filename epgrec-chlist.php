// 全国用 BS デジタルチャンネルマップ
// 作成者：https://github.com/autch/gen-chlist
// 作成日時：2013-12-24
// 参照元：http://www5e.biglobe.ne.jp/~kazu_f/digital-sat/trapon-bsat.html
// 参照元の更新日時：2012-07-01
$BS_CHANNEL_MAP = array(
  "3005.ontvjapan.com" => "151",		// BS朝日
  "3006.ontvjapan.com" => "161",		// BS-TBS
  "3009.ontvjapan.com" => "191",		// WOWOWプライム
  "3007.ontvjapan.com" => "171",		// BSジャパン
  "4192.epgdata.ontvjapan" => "192",		// WOWOWライブ
  "4193.epgdata.ontvjapan" => "193",		// WOWOWシネマ
  "4201.epgdata.ontvjapan" => "201",		// スター・チャンネル2
  "4202.epgdata.ontvjapan" => "202",		// スター・チャンネル3
  "1047.ontvjapan.com" => "236",		// BSアニマックス
  "1090.ontvjapan.com" => "256",		// ディズニー・チャンネル
  "3013.ontvjapan.com" => "211",		// BS11
  "3012.ontvjapan.com" => "200",		// スター・チャンネル1
  "3014.ontvjapan.com" => "222",		// TwellV
  "4238.epgdata.ontvjapan" => "238",		// FOX bs238
  "4241.epgdata.ontvjapan" => "241",		// BSスカパー！
  "4231.epgdata.ontvjapan" => "231",		// 放送大学1
  "4232.epgdata.ontvjapan" => "232",		// 放送大学2
  "4233.epgdata.ontvjapan" => "233",		// 放送大学3
  "3004.ontvjapan.com" => "141",		// BS日テレ
  "3008.ontvjapan.com" => "181",		// BSフジ
  "3001.ontvjapan.com" => "101",		// NHK BS1
  "3003.ontvjapan.com" => "103",		// NHK BSプレミアム
  "4910.ontvjapan.com" => "910",		// WNI
  "4291.epgdata.ontvjapan" => "291",		// NHK総合テレビジョン（東京）
  "4292.epgdata.ontvjapan" => "292",		// NHK教育テレビジョン（東京）
  "4294.epgdata.ontvjapan" => "294",		// 日本テレビ
  "4295.epgdata.ontvjapan" => "295",		// テレビ朝日
  "4296.epgdata.ontvjapan" => "296",		// TBSテレビ
  "4297.epgdata.ontvjapan" => "297",		// テレビ東京
  "4298.epgdata.ontvjapan" => "298",		// フジテレビ
  "4234.epgdata.ontvjapan" => "234",		// グリーンチャンネル
  "4242.epgdata.ontvjapan" => "242",		// J SPORTS 1
  "4243.epgdata.ontvjapan" => "243",		// J SPORTS 2
  "4252.epgdata.ontvjapan" => "252",		// IMAGICA BS
  "4244.epgdata.ontvjapan" => "244",		// J SPORTS 3
  "4245.epgdata.ontvjapan" => "245",		// J SPORTS 4
  "4251.epgdata.ontvjapan" => "251",		// BS釣りビジョン
  "4255.epgdata.ontvjapan" => "255",		// 日本映画専門チャンネル
  "4258.epgdata.ontvjapan" => "258",		// D-Life
);

// 全国用 CS デジタルチャンネルマップ
// 作成者：https://github.com/autch/gen-chlist
// 作成日時：2013-12-24
// 参照元：http://www5e.biglobe.ne.jp/~kazu_f/digital-sat/trapon-nsat110.html
// 参照元の更新日時：2013-10-01
$CS_CHANNEL_MAP = array(
  "3201.ontvjapan.com" => "CS2",		// 296: ＴＢＳチャンネル１
  "1092.ontvjapan.com" => "CS2",		// 298: テレ朝チャンネルＨＤ
  "1067.ontvjapan.com" => "CS2",		// 299: 朝日ニュースターＨＤ
  "100ch.epgdata.ontvjapan" => "CS4",		// 100: スカパー！プロモ
  "1217.ontvjapan.com" => "CS4",		// 227: ザ・シネマ
  "1008.ontvjapan.com" => "CS4",		// 223: チャンネルＮＥＣＯ
  "1040.ontvjapan.com" => "CS4",		// 250: ｓｋｙ・Ａスポーツ＋
  "1050.ontvjapan.com" => "CS4",		// 342: ヒストリーチャンネル
  "363ch.epgdata.ontvjapan" => "CS4",		// 363: 囲碁・将棋チャンネル
  "294ch.epgdata.ontvjapan" => "CS6",		// 294: ホームドラマＣＨ
  "329ch.epgdata.ontvjapan" => "CS6",		// 329: 歌謡ポップス
  "1019.ontvjapan.com" => "CS6",		// 323: ＭＴＶ　ＨＤ
  "1069.ontvjapan.com" => "CS6",		// 354: ＣＮＮｊ
  "1062.ontvjapan.com" => "CS6",		// 340: ディスカバリー
  "1193.ontvjapan.com" => "CS6",		// 341: アニマルプラネット
  "1059.ontvjapan.com" => "CS8",		// 055: ショップチャンネル
  "2002.ontvjapan.com" => "CS8",		// 349: 日テレＮＥＷＳ２４
  "1010.ontvjapan.com" => "CS8",		// 218: 東映チャンネル
  "1005.ontvjapan.com" => "CS8",		// 219: 衛星劇場
  "1024.ontvjapan.com" => "CS8",		// 326: ミュージック・エア
  "339ch.epgdata.ontvjapan" => "CS8",		// 339: ディズニージュニア
  "800ch.epgdata.ontvjapan" => "CS10",		// 800: スカチャン０
  "801ch.epgdata.ontvjapan" => "CS10",		// 801: スカチャン１
  "802ch.epgdata.ontvjapan" => "CS10",		// 802: スカチャン２
  "805ch.epgdata.ontvjapan" => "CS10",		// 805: スカチャン３
  "1026.ontvjapan.com" => "CS12",		// 254: ＧＡＯＲＡ
  "1022.ontvjapan.com" => "CS12",		// 325: エムオン！ＨＤ
  "1045.ontvjapan.com" => "CS12",		// 330: キッズステーション
  "1133.ontvjapan.com" => "CS14",		// 292: 時代劇専門ｃｈＨＤ
  "1015.ontvjapan.com" => "CS14",		// 293: ファミリー劇場ＨＤ
  "1006.ontvjapan.com" => "CS14",		// 310: スーパー！ドラマＨＤ
  "1207.ontvjapan.com" => "CS16",		// 290: ＳＫＹ　ＳＴＡＧＥ
  "305ch.epgdata.ontvjapan" => "CS16",		// 305: チャンネル銀河
  "1201.ontvjapan.com" => "CS16",		// 333: ＡＴ−Ｘ
  "1014.ontvjapan.com" => "CS16",		// 311: ＡＸＮ
  "1204.ontvjapan.com" => "CS16",		// 343: ナショジオチャンネル
  "1070.ontvjapan.com" => "CS16",		// 353: ＢＢＣワールド
  "1007.ontvjapan.com" => "CS18",		// 240: ムービープラスＨＤ
  "1027.ontvjapan.com" => "CS18",		// 262: ゴルフネットＨＤ
  "1074.ontvjapan.com" => "CS18",		// 314: 女性ｃｈ／ＬａＬａ
  "1073.ontvjapan.com" => "CS20",		// 307: フジテレビＯＮＥ
  "1072.ontvjapan.com" => "CS20",		// 308: フジテレビＴＷＯ
  "309ch.epgdata.ontvjapan" => "CS20",		// 309: フジテレビＮＥＸＴ
  "1120.ontvjapan.com" => "CS22",		// 161: ＱＶＣ
  "297ch.epgdata.ontvjapan" => "CS22",		// 297: ＴＢＳチャンネル２
  "1076.ontvjapan.com" => "CS22",		// 351: ＴＢＳニュースバード
  "1016.ontvjapan.com" => "CS22",		// 312: ＦＯＸ
  "1018.ontvjapan.com" => "CS22",		// 322: スペースシャワーＴＶ
  "1046.ontvjapan.com" => "CS22",		// 331: カートゥーン
  "229ch.epgdata.ontvjapan" => "CS24",		// 229: ＦＯＸムービー
  "1068.ontvjapan.com" => "CS24",		// 257: 日テレＧ＋　ＨＤ
  "300ch.epgdata.ontvjapan" => "CS24",		// 300: 日テレプラス
  "321ch.epgdata.ontvjapan" => "CS24",		// 321: スペシャプラス
  "1052.ontvjapan.com" => "CS24",		// 362: 旅チャンネル
);

